...
from .dhmamba_hi import DHMamba_hi
from torch.optim.lr_scheduler import LinearLR, CosineAnnealingLR, SequentialLR

class DHMamba_hi_pl(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.model_hparams = config.model
        self.opt_hparams = config.optimizer
        self.model = DHMamba_hi(num_classes=int(self.model_hparams['num_classes']), 
                                trans_dim=int(self.model_hparams['trans_dim']),  
                                num_group=self.model_hparams['num_group'], 
                                group_size=self.model_hparams['group_size'], 
                                avg_dist=float(self.model_hparams['avg_chamfer_dist']), 
