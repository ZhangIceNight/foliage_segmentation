...
class VoxelGroup(nn.Module):
    ...
    def forward(self, xyz):
        ...
        xyz: [B, N, 3]
        return:
            voxel_points: list of list, voxel_points[b][g] -> [M_i, 3]
            voxel_centers: list of tensors, voxel_centers[b] -> [G, 3]
        """
        B, N, _ = xyz.shape
        all_batches = []
        all_centers = []
        ...
        for b in range(B):
            coords = torch.floor(xyz[b] / self.voxel_size).int()  # voxel 坐标
            unique, inverse = torch.unique(coords, dim=0, return_inverse=True)

            neighs = []
            centers = []
            for g in range(unique.size(0)):
                mask = (inverse == g)
                pts = xyz[b][mask]   # [M_i, 3]
                if pts.shape[0] < 4:  # 保底 4 个
                    pad_idx = torch.randint(0, pts.shape[0], (4 - pts.shape[0],), device=pts.device)
                    pts = torch.cat([pts, pts[pad_idx]], dim=0)
                neighs.append(pts)               # [M_i, 3], M_i 可变
                centers.append(pts.mean(dim=0))  # [3]
            all_batches.append(neighs)
            all_centers.append(torch.stack(centers, dim=0))

        return all_batches, all_centers
...
class VoxelEncoder(nn.Module):
    def __init__(self, m=6, min_points=4, encoder_channel=384):
        """
        m: voxel 划分数, G = m^3
        min_points: voxel 内最少点数
        encoder_channel: 输出 token channel
        """
        super().__init__()
        self.m = m
    ...
    def forward(self, xyz):
        ...
        # 1️⃣ 计算 voxel grid
        min_xyz = xyz.min(dim=1, keepdim=True)[0]  # [B,1,3]
        max_xyz = xyz.max(dim=1, keepdim=True)[0]  # [B,1,3]
        voxel_size = (max_xyz - min_xyz) / m
        voxel_size[voxel_size == 0] = 1e-6

        # 2️⃣ 计算每个点 voxel idx
        voxel_idx = ((xyz - min_xyz) / voxel_size).long()
        voxel_idx = torch.clamp(voxel_idx, 0, m-1)
        voxel_id = voxel_idx[...,0]*m*m + voxel_idx[...,1]*m + voxel_idx[...,2]  # [B,N]

        # 3️⃣ scatter 聚合 voxel 内点坐标
        voxel_sum = torch.zeros(B, G, 3, device=xyz.device)
        voxel_count = torch.zeros(B, G, device=xyz.device)
        voxel_sum.scatter_add_(1, voxel_id.unsqueeze(-1).expand(-1,-1,3), xyz)
        voxel_count.scatter_add_(1, voxel_id, torch.ones_like(voxel_id, dtype=torch.float))
        ...
        for i, pts in enumerate(all_voxel_points):
            L = pts.shape[0]
            voxel_tensor[i,:L] = pts
            mask[i,:L] = 1

        # 6️⃣ PointNet encoder
        bs_g, n, _ = voxel_tensor.shape
        feature = self.first_conv(voxel_tensor.transpose(2,1))  # [B*G,256,N]
...
class DHMamba_voxel(nn.Module):
    def __init__(self, num_classes, HGNeighbors=2):
        ...
        self.voxel_encoder = VoxelEncoder(m=int(round(self.num_group ** (1/3))), min_points=4, encoder_channel=self.group_size)
    ...
    def forward(self, pts):
        B, N, C = pts.shape
        group_input_tokens, center = self.voxel_encoder(pts)  # [B, G, encoder_dims=384], [B, G, 3]
