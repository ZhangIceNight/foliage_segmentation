...
from .dhmamba_mse import DHMamba_mse
from torch.optim.lr_scheduler import LinearLR, CosineAnnealingLR, SequentialLR
import numpy as np
import os
class DHMamba_mse_pl(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.model_hparams = config.model
        self.opt_hparams = config.optimizer
        if config.model.get('dist') == 'volume':
            self.model = DHMamba_mse(num_classes=int(self.model_hparams['num_classes']), trans_dim=int(self.model_hparams['trans_dim']),  num_group=int(self.model_hparams['num_group']), group_size=int(self.model_hparams['group_size']), avg_dist=float(self.model_hparams['avg_volume_dist']), alpha=float(self.model_hparams['alpha']), HGNeighbors=int(self.model_hparams['HGNeighbors']))
        elif config.model.get('dist') == 'chamfer':
            self.model = DHMamba_mse(num_classes=int(self.model_hparams['num_classes']), trans_dim=int(self.model_hparams['trans_dim']),  num_group=int(self.model_hparams['num_group']), group_size=int(self.model_hparams['group_size']), avg_dist=float(self.model_hparams['avg_chamfer_dist']), alpha=float(self.model_hparams['alpha']), HGNeighbors=int(self.model_hparams['HGNeighbors']))
    ...
    def save_predictions(self, file_names, preds, points, labels, save_root="./Results/visualization_results/DHMamba"):
        """
        ...
        save to: ./Results/visualization_results/DHMamba/Larch/tte1.txt
        """
    ...
    def validation_step(self, batch, batch_idx):
        points, labels, avg_dist, file_names = batch
        logits = self.model(points, avg_dist)
