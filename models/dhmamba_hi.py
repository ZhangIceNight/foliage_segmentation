...
class Group(nn.Module):
    def __init__(self, num_group, group_size, avg_dist=None, alpha=2.0):
        ...
        for i in range(len(group_size)):
            self.knns.append(
                KNN(k=group_size[i], transpose_mode=True)
            )
    ...
    def forward(self, xyz):
        ...
        neighborhoods = []
        # fps the centers out
        for i in range(len(self.num_group)):
            center = fps(xyz, self.num_group[i])
            centers.append(center)
            # knn to get the neighborhood
            neighborhood = self._get_neighborhood(xyz, center, self.group_size[i])
...
class DHMamba_hi(nn.Module):
    def __init__(self, num_classes=2, trans_dim=384, num_group=[512,256,128,64], group_size=[8,16,32,64], avg_dist=0.5, alpha=2.0, HGNeighbors=4):
        super().__init__()
