...
def hungarian_algorithm_gpu(scores):
    """
    GPU版本的匈牙利算法
    Args:
        scores: [B, G, G] tensor on GPU
    Returns:
        assignment: [B, G, G] tensor on GPU
    """
    B, G, _ = scores.shape
    ...
    # 2. 行列归约
    cost = cost - cost.min(dim=-1, keepdim=True)[0]
    cost = cost - cost.min(dim=-2, keepdim=True)[0]
    
    # 初始化匹配数组
    row_ind = torch.full((B, G), -1, device=device, dtype=torch.long)
    col_ind = torch.full((B, G), -1, device=device, dtype=torch.long)
    
    def find_zeros_and_match(cost_matrix, row_ind, col_ind):
        """找到零元素并进行匹配"""
        zero_mask = (cost_matrix.abs() < 1e-8)
        unmatched_rows = (row_ind == -1)
        unmatched_cols = (col_ind == -1)
        
        # 优先选择每行/列零元素最少的位置
        zero_counts_row = zero_mask.float().sum(dim=-1)  # 每行的零元素数量
        zero_counts_col = zero_mask.float().sum(dim=-2)  # 每列的零元素数量
        
        # 计算每个零元素的权重（零元素越少权重越高）
        weights = (1.0 / (zero_counts_row.unsqueeze(-1) + zero_counts_col.unsqueeze(-2) + 1e-9))
        weights = weights.masked_fill(~zero_mask, 0)
        
        # 按权重大小排序选择零元素
        while True:
            if not (unmatched_rows.any() and unmatched_cols.any()):
                break
                
            # 找到权重最大的未匹配零元素
            valid_weights = weights * unmatched_rows.unsqueeze(-1).float() * unmatched_cols.unsqueeze(-2).float()
            if not valid_weights.any():
                break
                
            max_weight_pos = valid_weights.reshape(-1).argmax()
            i, j = max_weight_pos // G, max_weight_pos % G
            
            # 更新匹配
            row_ind[i] = j
            col_ind[j] = i
            unmatched_rows[i] = False
            unmatched_cols[j] = False
            
        return row_ind, col_ind
    
    def update_cost(cost_matrix, covered_rows, covered_cols):
        """更新cost矩阵"""
        # 找到未被覆盖的最小值
        mask = ~(covered_rows.unsqueeze(-1) | covered_cols.unsqueeze(-2))
        min_val = cost_matrix.masked_fill(~mask, 1e9).min()
        
        # 更新cost矩阵
        cost_matrix = cost_matrix.clone()
        cost_matrix[mask] -= min_val
        cost_matrix[covered_rows.unsqueeze(-1) & covered_cols.unsqueeze(-2)] += min_val
        
        return cost_matrix, min_val
    
    # 主循环
    for _ in range(G * 2):  # 最多需要2N次迭代
        # 对每个batch并行处理
        for b in range(B):
            # 1. 寻找零元素并匹配
            row_ind[b], col_ind[b] = find_zeros_and_match(cost[b], row_ind[b], col_ind[b])
            
            # 2. 如果找到完整匹配，继续下一个batch
            if (row_ind[b] != -1).all():
                continue
                
            # 3. 否则更新cost矩阵
            covered_rows = (row_ind[b] != -1)
            covered_cols = (col_ind[b] != -1)
            cost[b], min_val = update_cost(cost[b], covered_rows, covered_cols)
            
            # 如果最小值无效，说明出现了问题
            if min_val == 1e9:
                raise RuntimeError(f"No valid solution found for batch {b}")
    
    # 构建最终的assignment矩阵
    assignment = torch.zeros_like(cost, dtype=torch.float32)
    batch_indices = torch.arange(B, device=device).unsqueeze(-1).expand(-1, G)
    row_indices = torch.arange(G, device=device).unsqueeze(0).expand(B, -1)
    assignment[batch_indices, row_indices, row_ind] = 1
    assignment = assignment.transpose(-2, -1)
...
class get_model(nn.Module):
    def __init__(self, cls_dim):
        ...
        self.base_rotation_matrix = torch.eye(self.num_group)
        # define the encoder
    ...
    def get_permutation_matrix(self, H):
        """
        使用超图H和固定对角矩阵生成变换矩阵
        Args:
            H: 超图矩阵 [B, 3G, 3G]
