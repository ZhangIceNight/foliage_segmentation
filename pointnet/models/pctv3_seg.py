...
class PointTransformerV3Seg(nn.Module):
    ...
    def forward(self, xyz):
        ...
        Returns:
            logits: (B, N, C) tensor of global logits
        """
        
        # 通过backbone
        point = self.backbone(xyz)
        ...
        points_per_batch = xyz.shape[1]
        logits = feat.reshape(batch_size, points_per_batch, -1) # (B, N, C)
...
class get_model(nn.Module):
    def __init__(self, cls_dim):
        ...
        self.pctv3_backbone = PointTransformerV3Seg()
    ...
    def forward(self, pts):
        ...
        global_feature = self.pctv3_backbone(pts) # [B, N, C=512]


        f_level_0 = self.propagation_0(pts.transpose(-1, -2), pts.transpose(-1, -2), pts.transpose(-1, -2), global_feature) # [B, 3328, N]
