...
class get_model(nn.Module):
    def __init__(self, cls_dim):
        ...
        # 在三个G×G块上分别初始化为单位矩阵
        self.base_rotation_matrix[:self.num_group, :] = torch.eye(self.num_group)
        self.base_rotation_matrix[self.num_group:2*self.num_group, :] = torch.eye(self.num_group)
        self.base_rotation_matrix[2*self.num_group:, :] = torch.eye(self.num_group)
        # define the encoder
