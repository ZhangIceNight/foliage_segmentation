...
def main(args):
    ...
    experiment_dir = experiment_dir.joinpath('sem_seg_chinesescholartree')
    experiment_dir.mkdir(exist_ok=True)
    ...
    root = 'data_chinesescholartree/'
    NUM_CLASSES = 2
    NUM_POINT = args.npoint
    BATCH_SIZE = args.batch_size

    print("start loading training data ...")
    TRAIN_DATASET = ChineseWoodDataset(root=root, split='train', block_points=NUM_POINT)
    print("start loading test data ...")
    TEST_DATASET = ChineseWoodDataset(root=root, split='test', block_points=NUM_POINT)
